          ...
          <div className="space-y-6">
            ...
            <GradientCard gradient="purple-blue" className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold text-white">Your Trade Block</h3>
                <button className="px-4 py-2 bg-fantasy-purple-600 hover:bg-fantasy-purple-700 text-white rounded-lg text-sm transition-colors">
                  Manage Block
                </button>
              </div>
              <p className="text-gray-400 mb-4">Mark players you're willing to trade</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {userTeam?.roster.slice(0, 3).map((rosterPlayer) => (
                  <div key={rosterPlayer.id} className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p className="font-semibold text-white">{rosterPlayer.player.name}</p>
                        <p className="text-sm text-gray-400">{rosterPlayer.player.position} - {rosterPlayer.player.nflTeam}</p>
                      </div>
                      <StatusBadge variant="success" size="sm">Available</StatusBadge>
                    </div>
                    <div className="flex gap-2 mt-3">
                      <button className="flex-1 px-3 py-1 bg-slate-700 hover:bg-slate-600 text-white rounded text-xs transition-colors">
                        Remove
                      </button>
                      <button className="flex-1 px-3 py-1 bg-fantasy-blue-600 hover:bg-fantasy-blue-700 text-white rounded text-xs transition-colors">
                        View Offers
                      </button>
                    </div>
                  </div>
            ...
            <GradientCard gradient="dark" className="p-6">
              <h3 className="text-xl font-bold text-white mb-4">League Trade Block</h3>
              <p className="text-gray-400 mb-4">Players available for trade across the league</p>
              
              <div className="space-y-4">
                {teams.slice(1, 4).map((team) => (
                  <div key={team.id} className="bg-slate-800/30 rounded-lg p-4 border border-slate-700/50">
                    <div className="flex items-center gap-3 mb-3">
                      <TeamIcon teamName={team.name} />
                      <div>
                        <p className="font-semibold text-white">{team.name}</p>
                        <p className="text-xs text-gray-400">{team.User?.name || 'Manager'}</p>
                      </div>
                      <span className="ml-auto text-sm text-gray-400">{team.roster.length} available</span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {team.roster.slice(0, 4).map((rosterPlayer) => (
                        <span key={rosterPlayer.id} className="px-3 py-1 bg-slate-700 text-sm text-white rounded-full">
                          {rosterPlayer.player.name}
                        </span>
                      ))}
                    </div>
                    <button className="w-full mt-3 px-4 py-2 bg-fantasy-blue-600 hover:bg-fantasy-blue-700 text-white rounded-lg text-sm transition-colors">
                      Propose Trade
                    </button>
                  </div>
                ))}
            ...
            {/* Trade Block Tips */}
            <GradientCard gradient="dark" className="p-6">
              <h3 className="text-lg font-bold text-white mb-4">Trade Block Tips</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  <h4 className="font-semibold text-fantasy-purple-400 mb-2">Strategic Moves</h4>
                  <p className="text-gray-400">Put players on the block when their value is high or you need to upgrade other positions</p>
                </div>
                <div>
                  <h4 className="font-semibold text-fantasy-blue-400 mb-2">Market Value</h4>
                  <p className="text-gray-400">Check league trade block to gauge what other managers are seeking</p>
                </div>
                <div>
                  <h4 className="font-semibold text-fantasy-green-400 mb-2">Communication</h4>
                  <p className="text-gray-400">Players on trade block signal you're open to offers without committing</p>
                </div>
                <div>
                  <h4 className="font-semibold text-fantasy-yellow-400 mb-2">Flexibility</h4>
                  <p className="text-gray-400">Update your trade block weekly based on performance and team needs</p>
                </div>
              </div>
            </GradientCard>
          </div>
