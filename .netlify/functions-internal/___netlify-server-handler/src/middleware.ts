...
export async function middleware(request: NextRequest) {
  // TEMPORARY: Disable all authentication to test deployment
  ...
  // Skip authentication entirely for health and status endpoints
  if (pathname === '/api/health' || pathname === '/api/status' || pathname === '/healthz' || pathname === '/api/test-simple') {
    return NextResponse.next();
  }
...
export const config = {
  matcher: [
    ...
    '/api/((?!auth|health|status).)*' // Protect API routes except auth, health, and status routes
  ]
};
