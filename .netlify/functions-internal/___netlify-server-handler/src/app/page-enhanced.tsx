...
function LiveScoresTicker() {
  ...
  const liveGames = [
    {
      team1: { name: 'Team Alpha', score: 89.2, logo: '🏆' },
      team2: { name: 'Team Beta', score: 76.8, logo: '⚡' },
      status: 'Q3 14:22',
      isLive: true
    },
    {
      team1: { name: 'Team Gamma', score: 112.4, logo: '🔥' },
      team2: { name: 'Team Delta', score: 95.6, logo: '🎯' },
      status: 'FINAL',
      isLive: false
    },
    {
      team1: { name: 'Team Epsilon', score: 67.1, logo: '⭐' },
      team2: { name: 'Team Zeta', score: 72.3, logo: '💎' },
      status: 'Q2 8:45',
      isLive: true
    }
  ];
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentGame((prev) => (prev + 1) % liveGames.length);
    }, 4000);
    return () => clearInterval(interval);
  }, [liveGames.length]);

  return (
    <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-xl p-6 text-white mb-8 overflow-hidden relative">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          <Activity className="h-5 w-5 text-green-400" />
          <span className="font-semibold">Live Scores</span>
        ...
        <span className="text-sm text-gray-300">Week 8</span>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex items-center space-x-4 flex-1">
            <div className="text-2xl">{liveGames[currentGame].team1.logo}</div>
            <div>
              <p className="font-semibold">{liveGames[currentGame].team1.name}</p>
              <p className="text-2xl font-bold">{liveGames[currentGame].team1.score}</p>
            </div>
          </div>
          
          <div className="text-center px-4">
            <p className={`text-sm font-medium px-3 py-1 rounded-full ${
              liveGames[currentGame].isLive 
                ? 'bg-red-600 text-white animate-pulse' 
                : 'bg-gray-700 text-gray-300'
            }`}>
              {liveGames[currentGame].status}
            </p>
          </div>
          
          <div className="flex items-center space-x-4 flex-1 justify-end">
            <div className="text-right">
              <p className="font-semibold">{liveGames[currentGame].team2.name}</p>
              <p className="text-2xl font-bold">{liveGames[currentGame].team2.score}</p>
            </div>
            <div className="text-2xl">{liveGames[currentGame].team2.logo}</div>
          </div>
        </motion.div>
      </AnimatePresence>
    </div>
