...
export async function processWaiverClaims(leagueId: string, week?: number): Promise<WaiverProcessingResult> {
  ...
  for (const [playerId, claims] of Array.from(claimsByPlayer.entries())) {
    ...
    for (const claim of sortedClaims) {
      ...
      // Process the claim
      try {
        await processIndividualClaim(claim, leagueId, currentWeek, usesFAAB);
...
async function processIndividualClaim(claim: any, leagueId: string, currentWeek: number, usesFAAB: boolean) {
